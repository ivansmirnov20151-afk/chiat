-- =============================================================================
-- 🪐 CYBER ENGINE V33.1 [👑 ULTRA PREMIUM ADAPTIVE EDITION]
-- =============================================================================
...
local SpecialUsers = {
	["daniil2024815"] = "🎥 YOUTUBER",
	["ZTE2049"] = "👑 ADMIN"
}
local isBypassUser = SpecialUsers[player.Name] ~= nil
...
-- 25 видов кастомных курсоров
local CursorList = {
	"rbxasset://textures/Cursors/KeyboardMouse/ArrowFarCursor.png",
	"rbxasset://textures/Cursors/KeyboardMouse/ArrowCursor.png",
	"rbxassetid://6070020810", "rbxassetid://6069945199", "rbxassetid://6070021966",
	"rbxassetid://6070022646", "rbxassetid://6070023419", "rbxassetid://6070024227",
	"rbxassetid://6070024958", "rbxassetid://6070025816", "rbxassetid://6070026601",
	"rbxassetid://6070027663", "rbxassetid://6070028442", "rbxassetid://6070029312",
	"rbxassetid://6070030069", "rbxassetid://6070030784", "rbxassetid://6070031547",
	"rbxassetid://6070032478", "rbxassetid://6070033245", "rbxassetid://6070034102",
	"rbxassetid://9747863158", "rbxassetid://9747863421", "rbxassetid://9747863690",
	"rbxassetid://9747863980", "rbxassetid://9747864210"
}
...
IntroStatus.Text = isBypassUser and "Обнаружен статус привилегий! Вход без кода..." or "Инициализация премиум интерфейса..."
...
local titleString = "🪐 THUNDER CYBER MM2"
if isBypassUser then
	titleString = titleString .. " | [" .. SpecialUsers[player.Name] .. "]"
end
	...
	if tPlr and tPlr.Character and tPlr.Character:FindFirstChild("HumanoidRootPart") and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
		...
		for i = 1, 50 do
			if tRoot and myRoot then
				...
				myRoot.Velocity = Vector3.new(999, 999, 999)
			end
			task.wait(0.02)
		end
		bg:Destroy()
...
addMenuCmd("💬 19. Фейк Чат Троллинг", true, function(tPlr)
	logToConsole("[ЧАТ ТРОЛЛИНГ] " .. selectedFriendName .. ": Я использую CYBER ENGINE V33.1! Скрипт просто топ!")
...
-- =============================================================================
-- 🖱️ НОВАЯ ПОЛНОЦЕННАЯ ВКЛАДКА КУРСОРОВ
-- =============================================================================
addButton(tCursors, "❌ Сбросить на Обычный Курсор", function()
	mouse.Icon = ""
	logToConsole("Курсор сброшен на стандартный")
end)

for i, cursorId in ipairs(CursorList) do
	addButton(tCursors, "🖱️ Включить Кастомный Курсор #" .. i, function()
		mouse.Icon = cursorId
		logToConsole("Установлен кастомный курсор вид #" .. i)
	end)
end
		...
		for _, enemy in ipairs(Players:GetPlayers()) do
			if enemy ~= player and enemy.Character and enemy.Character:FindFirstChild("HumanoidRootPart") then
				...
				if States["EspNames"] then
					...
					local prefix = ""
					if SpecialUsers[enemy.Name] then
						prefix = "[" .. SpecialUsers[enemy.Name] .. "] "
					end
...
task.spawn(function()
	task.wait(4)
	logToConsole("🪐 CYBER ENGINE: Загрузка завершена.")
	...
	if isBypassUser then
		createNotification("🪐 ВХОД ВЫПОЛНЕН", "Добро пожаловать, привилегированный пользователь!", "Info")
		if KeySystemFrame then KeySystemFrame:Destroy() end
		MainFrame.Visible = true
		ClockWidget.Visible = true
		CounterWidget.Visible = true
		ToggleMenuBtn.Visible = true
		logToConsole("🪐 CYBER ENGINE: Успешный вход без кода подтверждения!")
	else
